      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-[350px] sm:w-[400px] h-[500px] mb-4  bg-white/95 backdrop-blur-xl border  border-indigo-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-sm ${msg.role === 'user' ? ' bg-indigo-500' : ' bg-indigo-100'}`}>
                    {msg.role === 'user' ? <User className="w-4 h-4  text-slate-800" /> : <Bot className="w-4 h-4  text-indigo-600" />}
                  </div>
                  <div className={`p-3 rounded-2xl shadow-sm max-w-[80%] ${msg.role === 'user' ? ' bg-indigo-500  text-slate-800 rounded-tr-none' : ' bg-gray-50 border  border-gray-200 rounded-tl-none'}`}>
            ...
            <div className="p-3  bg-white border-t  border-indigo-100 flex gap-2">
              <input 
                ...
                className="flex-1  bg-gray-50 border  border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-neonCyan transition"
              />
              <button 
                onClick={handleSend}
